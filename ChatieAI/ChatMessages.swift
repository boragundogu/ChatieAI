...
class ChatMessages {
    ...
    func sendMessage(_ message: String) {
        ...
        if(chat==nil) {
            ...
            chat = GenerativeModel(name: "gemini-pro", apiKey: Bundle.main.infoDictionary?["API_KEY"] as? String ?? "api key").startChat(history: history)
        }
        ...
        Task {
            do {
                ...
                guard let text = response?.text else {
                    messages.append(.init(role: .model, message: "Error, pls try again."))
                    return
                }
